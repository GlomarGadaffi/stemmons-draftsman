...
If you're here because **your panel is black but the backlight is on**, jump straight to **[The disp_on_off Inversion](disp_on_off-Inversion.md)**. That's the one that costs everyone a day.

## Pages

- **[The disp_on_off Inversion](disp_on_off-Inversion.md)** — the black-screen killer. Read this first.
...
```c
esp_lcd_panel_disp_on_off(panel, false);  // false == ON. This driver's bool means "off".
```

```text
✔ disp_on_off(false) to turn ON       ✔ LEDC PWM backlight (not gpio level)
